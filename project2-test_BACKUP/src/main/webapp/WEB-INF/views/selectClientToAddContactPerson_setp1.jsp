...
<div style="height:100%;display:flex; justify-content:center; align-items:center;      ;border-bottom-width:0px;">
 <div style="height:250px;width:250px;display:flex;flex-direction:column;flex-direction:column;justify-content:center;align-items:center;  box-shadow:0px 20px 40px 0px black;border-radius:20px">
...
List<String> listOfClientId=DAOImpl.getAllClientId();
List<String> listOfClientName=DAOImpl.getAllClientName();
if(listOfClientId!=null && listOfClientName!=null){
				System.out.println("putting client in suggestion in selectClient.jsp");
%>


   <h4>Select Client by Name or ID</h4></br>
<form action="addingContactPersonToClient_step2" method="get" onsubmit="return validateInput()">
    <input type="search" id="searchBox" name="clientName" placeholder="Search..." onkeyup="showSuggestions()">
    <div id="suggestions" class="autocomplete-suggestions"></div>
    <input type="hidden" id="inJsCode" name="clientId" value="">
...
<script>
    ...
    for (int i = 0; i < listOfClientId.size(); i++) {
        String clientName = listOfClientName.get(i);
        String ClientId = listOfClientId.get(i);
        out.print("words.push('" + clientName + "');\n");
        out.print("numericValues.push('" + ClientId + "');\n");
    }
    %>
    ...
    document.addEventListener("click", function (event) {
        ...
        if (!searchBox.contains(event.target) && !suggestionsDiv.contains(event.target)) {
            suggestionsDiv.style.display = "none";
				...
				//when their is no client in cleint atble in DB
				System.out.println("when their is no client in cleint atble in DB then using JS TOSTIFY notification");
				%>
				
				  <h4>Search Client by Name or ID</h4></br>
<form action="#" method="get" onsubmit="return validateInput()">
    <input type="search" id="searchBox" name="clientName" placeholder="Search..." onkeyup="showSuggestions()" readonly>
    <div id="suggestions" class="autocomplete-suggestions"></div>
    <input type="hidden" id="inJsCode" name="clientId" value="">
    <input type="submit" value="Submit">
    <p id="error-message" class="error-message">Invalid input! Please select a suggestion.</p>
...
<%--when their is no client in cleint atble in DB then using JS TOSTIFY notification --%>
<script>
...
<%--((((END))) when their is no client in cleint atble in DB then using JS TOSTIFY notification --%>				
